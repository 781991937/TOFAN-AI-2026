...
function showView(view){$(".nav-btn").forEach(b=>b.classList.toggle("active",b.dataset.view===view));["curriculumPanel","coursePanel","lessonPanel","notificationsPanel","accessPanel","ownerPanel","assessmentPanel","resultsPanel"].forEach(id=>{const e=$("#"+id);if(e)e.classList.add("hidden")});if(view==="home")return;if(view==="curriculum")openCurriculum();if(view==="progress")loadProgress();if(view==="assessments"&&window.loadAssessments)window.loadAssessments();if(view==="results"&&window.loadAssessmentResults)window.loadAssessmentResults();if(view==="certificates")loadCertificates();if(view==="profile")loadProfile();if(view==="notifications")loadNotifications();if(view==="access"&&window.loadAccess)window.loadAccess();if(view==="owner"&&isOwner)openOwnerManager()}
