...
function webauthnOptions(value){if(!value||typeof value!=="object")return value;const out=Array.isArray(value)?value.map(webauthnOptions):{...value};for(const k of Object.keys(out)){if(["challenge","user","rawId","id"].includes(k)&&typeof out[k]==="string"&&k!=="id"||k==="challenge"&&typeof out[k]==="string")out[k]=b64ToBuf(out[k]);else if(k==="user"&&out[k]?.id)out[k]={...out[k],id:b64ToBuf(out[k].id)};else if(k==="allowCredentials"||k==="excludeCredentials")out[k]=out[k].map(x=>({...x,id:b64ToBuf(x.id)}));}return out}
