...
def _enc(value: str) -> str: return hashlib.sha256(str(value).encode("utf-8")).hexdigest()[:12]
def _rows(buttons, width: int = 2): return [buttons[i:i + width] for i in range(0, len(buttons), width)]
def main_menu(): return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🧠 قسم الذكاء الاصطناعي",callback_data="ai_section"),InlineKeyboardButton(text="🤖 قسم البوت",callback_data="bot_section")],[InlineKeyboardButton(text="⚙️ إعدادات البوت",callback_data="settings"),InlineKeyboardButton(text="❓ المساعدة",callback_data="help")]])
def section_menu(section:str):
    library="ai_library" if section=="ai" else "bot_library"; return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="📚 المكتبة",callback_data=library),InlineKeyboardButton(text="📥 إدارة الملفات",callback_data="file_management")],[InlineKeyboardButton(text="🧭 الصفحات والتنقل",callback_data="navigation_help"),InlineKeyboardButton(text="⬅️ الرئيسية",callback_data="home")]])
def ai_categories_menu(c):
    rows=_rows([InlineKeyboardButton(text=f"{r['category']} ({int(r['lesson_count'])})",callback_data=f"ai_category:{_enc(str(r['category']))}") for r in c]); rows.append([InlineKeyboardButton(text="⬅️ قسم الذكاء",callback_data="ai_section")]); return InlineKeyboardMarkup(inline_keyboard=rows)
def bot_categories_menu(c):
    rows=_rows([InlineKeyboardButton(text=f"{r['category']} ({int(r['lesson_count'])})",callback_data=f"bot_category:{_enc(str(r['category']))}") for r in c]); rows.append([InlineKeyboardButton(text="⬅️ قسم البوت",callback_data="bot_section")]); return InlineKeyboardMarkup(inline_keyboard=rows)
def ai_category_menu(cat):
    t=_enc(cat); return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="📂 ملفات القسم",callback_data=f"ai_categoryfiles:{t}"),InlineKeyboardButton(text="🎓 اختبار القسم كامل",callback_data=f"ai_categoryquiz:{t}")],[InlineKeyboardButton(text="⬅️ المكتبة",callback_data="ai_library")]])
def ai_files_menu(files,cat):
    rows=_rows([InlineKeyboardButton(text=f"📘 {n} ({c})",callback_data=f"ai_file:{_enc(k)}") for k,n,c in files]); rows.append([InlineKeyboardButton(text="⬅️ القسم",callback_data=f"ai_category:{_enc(cat)}")]); return InlineKeyboardMarkup(inline_keyboard=rows)
...
def file_list_menu(lessons,cat):
    g={}
    for l in lessons:
        k=str(l["file_id"] or l["file_path"] or l["file_name"]); g.setdefault(k,[str(l["file_name"]),0])[1]+=1
    rows=_rows([InlineKeyboardButton(text=f"📘 {n[:25]}{'...' if len(n)>28 else ''} ({c})",callback_data=f"bot_file:{_enc(k)}") for k,(n,c) in g.items()]); rows.append([InlineKeyboardButton(text="⬅️ الأقسام",callback_data="bot_library")]); return InlineKeyboardMarkup(inline_keyboard=rows)
def bot_lesson_list(lessons,key):
    b=[]
    for n,l in enumerate(lessons,1):
        t=str(l["file_name"] or f"الدرس {n}"); t=t.split(" - ",1)[1] if " - " in t else t; t=t[:22]+"..." if len(t)>25 else t; b.append(InlineKeyboardButton(text=f"📖 {n}: {t}",callback_data=f"bot_lesson:{int(l['id'])}:{_enc(key)}"))
    rows=_rows(b); rows.append([InlineKeyboardButton(text="⬅️ الملفات",callback_data=f"bot_fileback:{_enc(key)}")]); return InlineKeyboardMarkup(inline_keyboard=rows)
