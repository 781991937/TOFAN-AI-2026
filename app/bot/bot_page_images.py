...
def _virtual_pages(text: str) -> list[dict]:
    return [
        ...
        for number, body in page_parts(text, chars_per_page=VIRTUAL_PAGE_CHARS)
    ]
...
def _pages(lesson) -> list[dict]:
    ...
    try:
        ...
        if isinstance(cached, list) and cached and isinstance(cached[0], dict) and "page" in cached[0]:
            # DOCX has no reliable physical page boundaries after text extraction.
            # Rebuild old single-page caches into stable virtual pages so the
            # bot never gets stuck showing "1 من 1" for a long lesson.
...
def _keyboard(lesson_id: int, pages: list[dict], index: int) -> InlineKeyboardMarkup:
    ...
    # الرجوع يكون إلى قائمة دروس الملف، وليس إلى قائمة خيارات الدرس.
...
def _caption(lesson, index: int, total: int, rendered_original: bool = True) -> str:
    note = (
        "🖼️ الصفحة من الملف الأصلي بدون تلخيص أو إعادة تنسيق."
        if rendered_original
        else "🖼️ نسخة مرئية مرتبة من محتوى الصفحة."
    )
    return (
...
def _render_text_page(text: str) -> bytes:
    ...
    draw.text(
        (width - margin, height - margin + 5),
        "TOFAN AI • البوت والأتمتة — Python",
        font=small,
        fill="black",
        anchor="ra",
        direction="rtl",
...
async def bot_lesson_quiz_from_page(
    ...
    try:
        ...
        if callback.message:
            await callback.message.answer(
                f"⚠️ <b>تعذر إنشاء اختبار الدرس</b>\n{html.escape(str(exc))}"
            )
