...
class MainAgentOrchestrator:
    ...
    def _enabled_tool_names(self, db: Session, agent: Agent) -> list[str]:
        return [name for name in self.registry.names() if db.scalar(select(AgentTool.id).where(
            AgentTool.agent_id == agent.id, AgentTool.tool_name == name, AgentTool.enabled.is_(True)
        ))]

    def _live_model(self, db: Session, agent: Agent, user_text: str, actor_user_id: str | None,
                    history: list[AgentMessage] | None = None) -> dict:
        ...
        if not isinstance(provider, OpenAIResponsesProvider):
            return {"kind": "tool", "provider": first.provider, "model": first.model,
                    "tool_calls": [c["name"] for c in first.tool_calls], "outputs": tool_outputs}
    ...
    def run(self, db: Session, agent: Agent, actor_user_id: str | None, user_text: str,
        ...
        try:
            return self._live_model(db, agent, user_text, actor_user_id, history)
