...
def get_db_curriculum(curriculum_slug: str, db: Session = Depends(get_db)):
    curriculum = db.scalar(select(Curriculum).where(Curriculum.slug == curriculum_slug, Curriculum.status == "active"))
    if curriculum is None:
        raise HTTPException(status_code=404, detail="Curriculum not found.")

    stages = db.scalars(select(CurriculumStage).where(CurriculumStage.curriculum_id == curriculum.id).order_by(CurriculumStage.position)).all()
    result_stages = []
    for stage in stages:
        courses = db.scalars(select(CurriculumCourse).where(CurriculumCourse.stage_id == stage.id, CurriculumCourse.is_active.is_(True)).order_by(CurriculumCourse.position)).all()
        result_courses = []
        for course in courses:
            outcomes = db.scalars(select(LearningOutcome).where(LearningOutcome.course_id == course.id).order_by(LearningOutcome.position)).all()
            prerequisites = db.scalars(select(CoursePrerequisite).where(CoursePrerequisite.course_id == course.id)).all()
            result_courses.append({
                "id": course.id, "code": course.code, "name": course.name, "course_type": course.course_type,
                "outcomes": [x.statement for x in outcomes],
                "prerequisite_course_ids": [x.prerequisite_course_id for x in prerequisites],
            })
        result_stages.append({"id": stage.id, "code": stage.code, "name": stage.name, "position": stage.position, "courses": result_courses})

    projects = db.scalars(select(CurriculumProject).where(CurriculumProject.curriculum_id == curriculum.id).order_by(CurriculumProject.code)).all()
    tracks = db.scalars(select(ElectiveTrack).where(ElectiveTrack.curriculum_id == curriculum.id).order_by(ElectiveTrack.name)).all()
    return {
        "id": curriculum.id, "slug": curriculum.slug, "name": curriculum.name, "version": curriculum.version,
